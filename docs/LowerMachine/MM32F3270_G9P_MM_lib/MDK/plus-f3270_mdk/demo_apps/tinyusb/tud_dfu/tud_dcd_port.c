...
void dcd_int_handler(uint8_t rhport)
{
    ...
    if (flag & USB_INT_STALL)
    {
        ...
        USB_BufDesp_Xfer(&usb_bd_tbl.Table[0u][USB_Direction_OUT][usb_epmng_tbl[0u][USB_Direction_IN].odd_even], 1, usb_ep0_buffer, 64);
...
bool dcd_edpt_open            (uint8_t rhport, tusb_desc_endpoint_t const * desc_ep)
{
    ...
    usb_epmng_tbl[ep_index][ep_dir].max_packet_size = desc_ep->wMaxPacketSize.size;
