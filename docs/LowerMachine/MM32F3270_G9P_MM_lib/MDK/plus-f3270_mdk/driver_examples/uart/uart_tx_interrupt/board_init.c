...
void BOARD_InitLed(void)
{
    ...
    GPIO_WriteBit(BOARD_LED_GPIO_PORT, BOARD_LED_GPIO_PINS, 1u);
}
...
void BOARD_LedOnTimeout(uint32_t ticks)
{
    ...
    GPIO_WriteBit(BOARD_LED_GPIO_PORT, BOARD_LED_GPIO_PINS, 0u); /* led on. */
    if (ticks == 0u)
    {
...
void SysTick_Handler(void)
{
    if (systick_ticks >= BOARD_LED_TICK_IN_SYSTICK_TICKS)
    {
        ...
        if ( led_timeout_ticks != 0u)
        {
            ...
            {
                GPIO_WriteBit(BOARD_LED_GPIO_PORT, BOARD_LED_GPIO_PINS, 0u);
            }
            led_timeout_ticks--;
        }
    }
